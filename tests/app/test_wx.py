...
def test_mouse_press(evented_canvas: snx.Canvas) -> None:
    """Test that Wx mouse press events are converted and given to the canvas."""
    native = cast(
...
def test_mouse_release(evented_canvas: snx.Canvas) -> None:
    """Test that Wx mouse release events are converted and given to the canvas."""
    native = cast(
        "CanvasAdaptor", evented_canvas._get_adaptors(create=True)[0]
    )._snx_get_native()
    press_point = (5, 10)
...
def test_mouse_move(evented_canvas: snx.Canvas) -> None:
    """Test that Wx mouse move events are converted and given to the canvas."""
    native = cast(
        "CanvasAdaptor", evented_canvas._get_adaptors(create=True)[0]
    )._snx_get_native()
    press_point = (5, 10)
...
def test_mouse_wheel(evented_canvas: snx.Canvas) -> None:
    """Test that Wx wheel events are converted and given to the canvas."""
    native = cast(
        "CanvasAdaptor", evented_canvas._get_adaptors(create=True)[0]
    )._snx_get_native()
    press_point = (5, 10)
    with patch("scenex.model._canvas.Canvas.handle") as mock:
        _processEvent(
            wx.EVT_MOUSEWHEEL, native, pos=wx.Point(*press_point), rot=(0, 120)
        )
    mock.assert_called_once_with(
        WheelEvent(
            canvas_pos=press_point,
...
def test_resize(evented_canvas: snx.Canvas) -> None:
    """Test that Wx resize events are converted and given to the canvas."""
    native = cast(
        "CanvasAdaptor", evented_canvas._get_adaptors(create=True)[0]
    )._snx_get_native()
    new_size = (400, 300)
...
def test_mouse_enter(evented_canvas: snx.Canvas) -> None:
    """Test that Wx enter events are converted and given to the canvas."""
    native = cast(
        "CanvasAdaptor", evented_canvas._get_adaptors(create=True)[0]
    )._snx_get_native()
    enter_point = (0, 15)
...
def test_mouse_leave(evented_canvas: snx.Canvas) -> None:
    """Test that Wx leave events are converted and given to the canvas."""
    native = cast(
        "CanvasAdaptor", evented_canvas._get_adaptors(create=True)[0]
    )._snx_get_native()

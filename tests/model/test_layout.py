import fractions as stdlib_fractions

import scenex as snx
from scenex.model._layout import ComposedDim, Fraction, Layout, Pixel
...
def test_pixel_layout() -> None:
    ...
    view = snx.View()
    view.layout.x = Pixel(pixels=40), Pixel(pixels=80)
    view.layout.y = Pixel(pixels=40), Pixel(pixels=80)
    canvas = snx.Canvas(views=[view], width=100, height=100)
...
def test_negative_pixel_layout() -> None:
    ...
    view.layout.x = Pixel(pixels=-40), Fraction(num=1, denom=1)
    view.layout.y = Pixel(pixels=-40), Fraction(num=1, denom=1)
    canvas = snx.Canvas(views=[view], width=100, height=100)
...
def test_span_mixture() -> None:
    ...
    view.layout.x = Pixel(pixels=40), Pixel(pixels=-40)
...
def test_stretch_layout() -> None:
    ...
    view = snx.View()
    view.layout.x = Pixel(pixels=40), Pixel(pixels=-40)
    view.layout.y = Pixel(pixels=40), Pixel(pixels=-40)
    canvas = snx.Canvas(views=[view], width=100, height=100)
...
def test_fractional_full_canvas() -> None:
    """Fraction(0, 1) to Fraction(1, 1) fills the entire canvas and scales with it."""
    view = snx.View()
    view.layout.x = Fraction(num=0, denom=1), Fraction(num=1, denom=1)
    view.layout.y = Fraction(num=0, denom=1), Fraction(num=1, denom=1)
    canvas = snx.Canvas(views=[view], width=100, height=100)
    assert canvas.rect_for(view) == (0, 0, 100, 100)
    canvas.width = 200
    assert canvas.rect_for(view) == (0, 0, 200, 100)
...
def test_fractional_nonzero_start() -> None:
    ...
    view = snx.View()
    view.layout.x = Fraction(num=1, denom=2), Fraction(num=1, denom=1)
    canvas = snx.Canvas(views=[view], width=100, height=100)
...
def test_fractional_covers_all_pixels() -> None:
    ...
    thirds = [
        Fraction(num=r.numerator, denom=r.denominator)
        for r in (stdlib_fractions.Fraction(k, 3) for k in range(4))
    ]
    for k, v in enumerate(views):
        v.layout.x = thirds[k], thirds[k + 1]
...
def test_mixed_units() -> None:
    ...
    view.layout.x = half - Pixel(pixels=20), half + Pixel(pixels=20)
    canvas = snx.Canvas(views=[view], width=100, height=100)
...
def test_dim_arithmetic() -> None:
    ...
    assert (Fraction(num=1, denom=2) + Pixel(pixels=10)) == ComposedDim(
        dim1=Fraction(num=1, denom=2), dim2=Pixel(pixels=10), operand="add"
    )
    assert (Fraction(num=1, denom=1) - Pixel(pixels=40)) == ComposedDim(
        dim1=Fraction(num=1, denom=1), dim2=Pixel(pixels=40), operand="sub"
    )
    assert (-Pixel(pixels=20)) == Pixel(pixels=-20)
    assert (2 * Fraction(num=1, denom=4)) == Fraction(num=1, denom=2)


def test_layout_serialization() -> None:
    """Layout round-trips through JSON with Dim fields intact."""
    layout = Layout(x_start=Fraction(num=1, denom=4), x_end=Fraction(num=3, denom=4))
    ...
    half = Fraction(num=1, denom=2)
    layout = Layout(x_start=half - Pixel(pixels=200), x_end=half + Pixel(pixels=200))
    json_str = layout.model_dump_json()
    layout2 = Layout.model_validate_json(json_str)
